<template>
  <div>
    ...
    <el-table :data="treeList" stripe row-key="id" default-expand-all
      ...
      <el-table-column type="selection" width="42" :selectable="row => row.parentId > 0" />
      ...
      <el-table-column label="操作" width="160">
        <template #default="{ row }">
          <el-button v-if="hasPermission('menus:update')" size="small" @click="openDialog(row)">编辑</el-button>
          <el-button v-if="hasPermission('menus:delete')" size="small" type="danger" @click="onDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const load = async () => {
  ...
  if (res.code === 200) list.value = flattenTree(res.data)
}
